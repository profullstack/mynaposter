 ...
 * The myna MCP server.
 *
 * Gives an agent the same abilities the CLI has: see what accounts are
 * connected, draft, post, schedule and read back. It talks to the local vault,
 * so an agent can post as you without ever being handed a credential.
 *
 * Two deliberate constraints:
 *   - There is no login tool. Connecting an account means typing a password or
 *     completing a browser flow, and that belongs to a person, not an agent.
 *   - myna_post publishes publicly and cannot be undone from here for every
 *     network. It says so in its description, and dry_run exists to check the
 *     targets and the tailoring first.
 */
