...
<ul>
<li><code>https://www.googleapis.com/auth/calendar.events</code>: to create, update and delete the calendar events myna makes for posts you schedule to your calendar (so a scheduled post shows up at the right time and disappears if you cancel it), to add an event you ask for with <code>myna calendar add</code>, and to show your upcoming events in your terminal with <code>myna calendar list</code>.</li>
<li><code>https://www.googleapis.com/auth/calendar.calendarlist.readonly</code>: to list your calendars so you can pick which one myna writes to.</li>
</ul>
<p>myna reads events only to show you your own upcoming events in your terminal when you run <code>myna calendar list</code>; it does not store, upload or analyze them. The Google access and refresh tokens are stored only in the encrypted vault on your computer; they are not sent to mynaposter.com or myna cloud (unless you run <code>myna cloud push</code>, which uploads them sealed with a passphrase we never see). Google user data is not sold, not used for advertising, not used to train AI or machine learning models, and not shared with anyone except as needed to provide the calendar feature you turned on or as required by law.</p>
