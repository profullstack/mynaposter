...
export async function runNewsletter(positional: string[], flags: Flags): Promise<number> {
  ...
  switch (sub ?? "list") {
    ...
    case "sync-optouts": {
      ...
      out(`myna cloud: ${cloud.pulled} unsubscribe${cloud.pulled === 1 ? "" : "s"} read, ${cloud.optedOut.length} newly opted out${cloud.unknown ? `, ${cloud.unknown} for tokens this install never sent` : ""}.`);
      if (result.tracking)
